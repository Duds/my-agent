...
### 2. PBI Lifecycle
- **Creation**: When a "Must Have" item in `TODO.md` is ready for technical design, a PBI file is created in `.agent/backlog/`.
- **Naming Convention**: `PBI-[SECTION_CODE]-[Feature-Name].md` (e.g., `PBI-SR-Skills-Registry.md`).
- **Completion**: Once a PBI is fully implemented, it is moved to a `.agent/backlog/archive/` folder or marked as completed within the file.
...
2. Read any relevant `PBI-*.md` in `.agent/backlog/` for technical context.
